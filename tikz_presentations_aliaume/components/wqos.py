...
class WqoUtilite:
    def draw(self, pic):
        ...
        pic.node(r"\includegraphics[width=2cm]{./petri_net.png}",
                 at=(3,-6),
                 )
        ...
        pic.node(r"\includegraphics[width=2cm]{./graph_minor.png}",
                 at=(2,1),
                 anchor="center")
...
class HigmanSubword:
    ...
    def draw(self, pic : Picture):
        ...
        pic.draw((0, 4.5), node(r"L’ordre sous-mot de Higman",
                                font="\\huge\\scshape"))
        ...
        if not self.show_orders:
            sc.draw("(A0.north west)", rectangle(f"(A{small_size - 1}.south east)"), 
                     draw="C5")
    
        if self.show_big_word:
            for j, b in enumerate(self.big_word):
                posx = (j - big_size / 2) * coef
                sc.node(r"\strut\ " + b + r"\strut\ ",
                         at=(posx, 2), 
                         minimum_size="1.5em",
                         draw=True if self.show_orders else None,
                         circle=True if self.show_orders else None,
                         inner_sep="0pt",
                         align="left",
                         name=f"B{j}", anchor="base")

            if not self.show_orders:
                sc.draw("(B0.north west)", rectangle(f"(B{big_size - 1}.south east)"))

        if self.show_orders:
            for x, word, angle in [("A", self.small_word, "-90"), ("B", self.big_word, "90")]:
                for i in range(len(word)):
                    for j in range(i):
                        sc.draw(f"({x}{i})", topath(f"({x}{j})", _in=angle, out=angle),
                                 opacity= 1 / (i - j + 1),
        ...
        if self.show_big_word and self.show_embedding:
            ...
            sc.draw((0,-2), node(r"$\sigma = \{ (\leq, 2) \}$", 
                                 font="\\large"))
    ...
    def __iter__(self):
        yield (0, HigmanSubword(self.small_word, self.big_word, self.embedding, False, False, False, False))
        yield (1, HigmanSubword(self.small_word, self.big_word, self.embedding, True, False, False, False))
        yield (1, HigmanSubword(self.small_word, self.big_word, self.embedding, True, True, False, False))
        yield (1, HigmanSubword(self.small_word, self.big_word, self.embedding, True, True, True, False))
        yield (1, HigmanSubword(self.small_word, self.big_word, self.embedding, True, True, True, True))
...
class StateOfTheArt:
    ...
    def draw(self, pic: Picture):
        ...
        for (e, x, align, anchor, prefix) in [(entries_left, -10, "right", "west", "L"), (entries_right, 9.5, "left", "east", "R")]:
            for i, entry in enumerate(e):
                authors = ", ".join(entry["author"])
                pic.node(authors, at=(x,-i+2), 
                                          anchor=anchor,
                                          name=f"A{prefix}{i}",
                                          align=align,
                                          font="\\small",
                                          text_width="5cm")
                pic.node(entry["title"], at=(x,-i+1.5), 
                                          anchor=anchor,
                                          opacity=0.6,
                                          inner_sep="0pt",
                                          name=f"T{prefix}{i}",
        ...
        pic.draw((timeline_x,timeline_y), lineto((timeline_x + 8,timeline_y)), thick=True, opt="->")

        for i in range(first_year, last_year+1, 5):
            pic.draw((timeline_x + scale * (i - first_year), timeline_y -0.1), 
                     lineto((timeline_x+ scale * (i - first_year), timeline_y + 0.1)))
            pic.node(str(i), at=(timeline_x + scale * (i - first_year), timeline_y -0.3), anchor="center")

        for (e, anchor, prefix, _in, _out) in [(entries_left, "east", "L", "90", "0"), (entries_right, "west", "R", "90", "180")]:
            for i, entry in enumerate(e):
                year = int(entry["year"])

                x = timeline_x + scale * (year - first_year)
                pic.coordinate(f"D{prefix}{i}",at=(x, timeline_y))
                pic.draw(f"(A{prefix}{i}.{anchor})", topath(f"(D{prefix}{i})", _in=_in, out=_out), opt="->")
...
class FinitePaths:
    ...
    def draw(self, pic: Picture):
        ...
        if frame >= 1:
            ...
            for i in range(4):
                ...
                pic.node(f"", 
                         ...
                         fill=f"{color}hint",
                         name=f"A{i}",
                         at=(i, 0), minimum_size="1.5em",
                         circle=True, inner_sep="0pt")
        ...
        if frame == 2 or frame == 3:
            for i in range(4):
                pic.draw(f"(A{i})", topath(f"(B{i+1})"), opt="->", thick=True, color="C3")
        ...
        if frame >= 4:
            ...
            pic.node(r"\includegraphics[width=4cm]{path.jpg}",
                     at=(-4,-1),
                     anchor="center")
...
class GraphWqo:
    ...
    def draw(self, pic):
        ...
        pscope.node("Chemins",
                    ...
                    at=(-5,-3.8))
        pscope.node(r"$\simeq (\mathbb{N}, \leq)$",
                    at=(-5,2.2),
                    )
        pscope.draw((-8,2),
                    rectangle((-2,-3.5)),
...
class PouzetConjectures:
    ...
    def draw(self, pic):
        ...
        pic.draw((6,0.6), node(r"Est-ce que"))
        pic.draw("(Cinf.south)", node("+ couleurs WQO", anchor="north", text_width="2cm", align="center", color="A4"))
        pic.draw((6,-1.6), node(r"est WQO?", color="Prune"))
        ...
        if frame >= 1:
            ...
            pic.draw("(C2.south)",   node("+ 2 couleurs", anchor="north", text_width="3cm", align="center", color="A4"))
            pic.draw("(Cfin.south)", node("+ couleurs finies", anchor="north", text_width="2cm", align="center", color="A4"))
        ...
        if frame >= 2:
            pic.draw("(C)", topath("(C2)", bend_left="30"),
                     double=True,
                     opt="<-"
                     )
            pic.draw("(C2)", topath("(Cfin)", bend_left="30"),
                     double=True,
                     opt="<-"
                     )
            pic.draw("(Cfin)", topath("(Cinf)", bend_left="30"),
                     double=True,
                     opt="<-"
                     )
        ...
        if frame >= 3:
            ...
            pic.draw((-1,1), lineto((1,1)), double=True, opt="->", dashed=True, thick=True, color="B2")
            pic.draw((4,1.6), node("Conjecture 2", align="center", text_width="2cm"))
            pic.draw((3,1), lineto((5,1)), double=True, opt="->", dashed=True, thick=True, color="B2")

        if frame >= 4:
            pic.draw((-9.5,-1.5), node(r"\includegraphics[width=2cm]{pouzet.jpg}", 
                                   anchor="north west", text_width="2cm",
                                   align="center",
                                   rounded_corners="5pt"))
            pic.draw((-7, -3), 
                     node(r"\textbf{Conjecture de Pouzet (1)}: $2 = \omega$",
                          anchor="west",
                          align="left", text_width="15cm"))
            pic.draw((-7, -3.5), 
                     node(r"\textbf{Conjecture de Pouzet (2)}: $\omega = \mathsf{WQO}$", 
                          anchor="west",
                          align="left", text_width="15cm"))
            pic.draw((-7, -4), 
                     node(r"\textbf{Conjecture de Schmitz}: les chemins sont la seule obstruction",
                          anchor="west",
                          align="left", text_width="15cm"))
...
if __name__ == "__main__":
    preview_animation(Sequential(frames=[
       WQOWorks(
           nsquare=NSquareWqo(
